...
export default function Home({
    ...
    const filterAndSorttoolsDatabase = () => {
        ...
        const orderedtoolsDatabase = orderByThreeHighlights(
            filteredtoolsDatabase
        );
    ...
    const extractThreeRandomHighlights = (toolsDatabase) => {
        ...
        if (allHighlights.length >= 3) {
            for (let i = 0; i <= 2; i++) {
                ...
                let randomItem =
                    allHighlights[
                        Math.floor(Math.random() * allHighlights.length)
                    ];

                // Se o item puxado já estiver na lista dos três aleatórios, puxa outro até vir um único
                if (
                    i > 0 &&
                    threeRandom.some((item) => item.id === randomItem.id)
                ) {
                    while (
                        threeRandom.some((item) => item.id === randomItem.id)
                    ) {
    ...
    const orderByThreeHighlights = (toolsDatabase) => {
        ...
        const orderedtoolsDatabaseWithoutRandom =
            toolsDatabaseWithoutRandom.sort((a, b) => {
                    ...
                    {filteredToolsData.length > 0 ? (
                        <div className={styles.resultsContainer}>
                            <div className={styles.resultsInfo}>
                                ...
                                {filteredToolsData.length === 1
                                    ? "Resultado"
                                    : "Resultados"}
                            </div>
                            {filteredToolsData.filter((item) => item.destaque)
                                .length > 0 && (
                                <div
                                    className={styles.highlightsTitleContainer}
                                >
                                    <Icon
                                        icon="mdi:star"
                                        color={styles.yellow}
                            ...
                            <InfiniteScroll
                                ...
                                endMessage={
                                    <div
                                        className={
                                            styles.allResultsLoadedMessage
                                        }
                        ...
                        >
                            <strong>
                                Não foi encontrado nenhum resultado para esta
                                filtragem
                            </strong>
                        </div>
                    )}
